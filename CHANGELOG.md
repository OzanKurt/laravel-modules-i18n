...
- **Source scanner**: `ScanReport` support class and `GET /api/i18n/report/scan` endpoint that walk the configured source paths for translation call sites and cross-check every key found against the JSON and PHP files on disk. The report returns four categories in one pass: `missing` (keys the code calls that no locale defines, each entry carrying the key plus the store, group and package the resolver placed it in), `unused` (keys the catalogue defines that no call site references), `dynamic` (call sites whose key could not be read, e.g. a variable or interpolation) and `ambiguous` (a literal key that matches no known store). Reported paths are relative to `base_path()`. `unused` is withheld, with a warning saying why, whenever the scan cannot vouch for the whole walk: no literal call site was found at all, or any file failed to scan. `config('i18n.scan.ignored_groups')` and `config('i18n.scan.ignored_keys')` suppress `unused` only, and vendor namespaced keys (`package::group.item`) are withheld from `unused` unconditionally; neither ever hides a key from `missing`. Scan results are fingerprint-cached per file (path + mtime + size, keyed on the scan config) via `ScanCache`, with a `refresh` query parameter to flush the cache before scanning.
- **`i18n:scan` artisan command**: the package's first artisan command, a read-only CLI wrapper around `ScanReport` for running the source scan from CI without a web server or authentication. Supports `--only` (narrow the categories printed and gated), `--locales`, `--format` (`table` or `json`, with JSON always emitting the full report regardless of `--only` so it stays identical to the endpoint's `data` body, which the endpoint wraps in the `{ data, meta }` envelope and the CLI prints unwrapped), `--fail` (non-zero exit on findings, ignoring `dynamic` unless it is explicitly named), and `--refresh` (bypass the scan cache). Exits `2` whenever the scan could not vouch for the whole walk: a file failed to parse and `unused` was requested, or no literal translation call was found at all, which ignores `--only` because it leaves every category vacuous. Exits `64` on a usage error. Translation keys are printed verbatim in both formats, so console markup inside a key (`Press <info>enter</info>`) is escaped for the terminal instead of being interpreted, and a clean table run prints a `No findings.` line rather than nothing at all.
