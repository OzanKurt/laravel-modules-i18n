...
- **`i18n:scan` artisan command**: the package's first artisan command, a read-only CLI wrapper around `ScanReport` for running the source scan from CI without a web server or authentication. Supports `--only` (narrow the categories printed and gated), `--locales`, `--format` (`table` or `json`, with JSON always emitting the full report regardless of `--only` so it stays identical to the endpoint), `--fail` (non-zero exit on findings, ignoring `dynamic` unless it is explicitly named), and `--refresh` (bypass the scan cache). Exits `2` when the scan was incomplete and `unused` was requested, `64` on a usage error.
