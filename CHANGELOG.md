...
- **Source scanner**: `ScanReport` support class and `GET /api/i18n/report/scan` endpoint that walk the configured source paths for translation call sites and cross-check every key found against the JSON and PHP files on disk. The report returns four categories in one pass: `missing` (keys the code calls that no locale defines), `unused` (keys the catalogue defines that no call site references), `dynamic` (call sites whose key could not be read, e.g. a variable or interpolation) and `ambiguous` (a literal key that matches no known store). `config('i18n.scan.ignored_groups')` and `config('i18n.scan.ignored_keys')` suppress `unused` only, and vendor namespaced keys (`package::group.item`) are withheld from `unused` unconditionally; neither ever hides a key from `missing`. Scan results are fingerprint-cached per file (path + mtime + size, keyed on the scan config) via `ScanCache`, with a `refresh` query parameter to flush the cache before scanning.
